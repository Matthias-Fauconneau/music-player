...
fn write <S: sample::Sample+'static, D, Output: std::ops::DerefMut<Target=[self::Output; N]>, const N: usize>
...
fn main() -> Result {
	...
	let playlist = playlist.into_iter().filter_map(|entry| { let path = entry.path(); println!("{}", path.display()); open(path).ok() });
	let stop = AtomicBool::new(false);
	let stop = &stop;
	let trigger = ui::new_trigger()?;
	let trigger = &trigger;
	std::thread::scope(|s| {
		std::thread::Builder::new().spawn_scoped(s, {let player : Arch<Player> = Arch::clone(&player); move || /*Result::<()>::unwrap(try*/ {
			for (mut reader, metadata, mut decoder) in playlist {
				...
				let output = || MutexGuard::map(player.lock(), |unlocked_player| <&mut [Output; N]>::try_from(unlocked_player.output.as_mut_slice()).unwrap());
				match sample_format {
				...
				if next.load(Relaxed) { next.store(false, Relaxed); }
			}
		}/*)*/})?;
		let r = ui::run(trigger, "Player", Box::new(|_,_| Ok(Box::new(player)))).inspect_err(|e| println!("{e:?}"));
...
#[cfg(feature="ui")] impl Widget for Player<'_> {
	fn paint(&mut self, context: &Context, commands: &mut Commands, target: Arc<ImageView>, size: size, _: int2) -> Result {
		...
		let image = image(context, commands,
			...
			//Image::from_xy(size, |image::xy{x,y}| rgba8{r: if x%2==0 { 0 } else { 0xFF }, g: if y%2==0 { 0 } else { 0xFF }, b: 0xFF, a: 0xFF})
			.as_ref())?;
