use alsa::{Result, PCM, direct::pcm::MmapPlayback, pcm::{HwParams,Format,Access,Frames,State}, ValueOr};
...
impl Output {
	pub fn new(rate: u32) -> Result<Self> {
		let device = PCM::new("hw:0,0", alsa::Direction::Playback, false)?;
	...
	pub /*async*/ fn write(&self, frames: &mut impl ExactSizeIterator<Item=(i16, i16)>) -> Result<usize> {
		assert!(frames.len() > 0);
		let ref fd = unsafe{std::os::fd::BorrowedFd::borrow_raw(alsa::PollDescriptors::get(&self.device)?[0].fd)};
		let ref mut fds = [rustix::io::PollFd::new(fd, rustix::io::PollFlags::OUT)];
		rustix::io::poll(fds, -1).unwrap(); // Block audio thread
