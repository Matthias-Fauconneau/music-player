...
impl super::Player<'static> {
    ...
    fn play(&self) { self.device.pause(false).unwrap(); }
    fn play_pause(&self) { super::toggle_play_pause(&self.device).unwrap(); }
    fn previous(&self) {}
    ...
    #[dbus_interface(property)] fn can_pause(&self) -> bool { super::playing(&self.device) }
    ...
    #[dbus_interface(property)] fn playback_status(&self) -> String { if super::playing(&self.device) {"Playing"} else {"Paused"}.into()  }
